...
steps:
  ...
  derep:
    run: uparseDerepWorkAround-docker.cwl
    in:
      fastaFiles: filter/filteredFasta
    out:  [ derepFasta ]

  sort:
    run: uparseSort-docker.cwl
    in:
  ...
  chimeraCheck:
    run: uparseChimeraCheck-docker.cwl
    in:
  ...
  renameOTU:
    run: uparseRenameOTUs-docker.cwl
    in:
      fastaFile: chimeraCheck/chimeraCleanFasta
    out: [ renamedFasta ]

  concatFasta:
    run: concatFasta-docker.cwl
    in:
     fastaFiles: filter/filteredFasta
    out: [ concatFasta ]

  underep:
    run: uparseGlobalSearchWorkAround-docker.cwl
    in:
  ...
  uparseUCtoTab:
    run: uparseOtuToTab-docker.cwl
    in:
      ucTabbed: underep/ucTabbed
    out: [ otuTable ]

  otuTableToBiom:
    run: qiimeOtusTxt2Biom-docker.cwl
    in:
      otuTable: uparseUCtoTab/otuTable
    out: [ otuBiom ]

  assignTaxonomy:
    run: qiimeAssignTaxonomy-docker.cwl
    in:
  ...
  addTaxonomyToBiom:
    run: qiimeAddMetadata-docker.cwl
    in:
